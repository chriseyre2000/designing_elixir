defmodule QuestionTest do
  ...
  test "building creates ask question text" do
    question = build_question(generators: addition_generators([1], [2]))
    assert question.asked == "1 + 2"
  end

  test "a random choice is made from the generators" do
  ...
  def eventually_match(generators, number) do
    ...
    |> Enum.find( fn substitution -> Keyword.fetch!(substitution, :left) == number end)
  end
end